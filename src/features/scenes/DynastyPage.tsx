...
import { useDynastyInfo } from './useDynastyInfo'
...
export function DynastyPage() {
  ...
  const numericDynastyId = Number(dynastyId)
  const { poets, loading } = usePoets(numericDynastyId)
  const { dynasty, styleClass, bgClass } = useDynastyInfo(numericDynastyId)
  const { user } = useUser()
        ...
        <div className="max-w-md mx-auto space-y-3">
          {regularPoets.map((p, i) => {
            ...
            return (
              <Link
                key={p.id}
                to={`/app/dynasty/${dynastyId}/challenge/${p.id}`}
                className="block bg-bg-card rounded-xl border border-border hover:border-[var(--dynasty-primary)] p-4 transition-colors"
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full flex items-center justify-center" style={{ backgroundColor: 'color-mix(in srgb, var(--dynasty-primary, var(--accent)) 10%, transparent)' }}>
                      <span className="font-serif font-bold" style={{ color: 'var(--dynasty-primary, var(--accent))' }}>{p.name.charAt(0)}</span>
                    </div>
                    <div>
          ...
          {bossPoet && (
            <div className="mt-6 pt-6 border-t border-border-light">
              {allRegularCompleted ? (
                <Link
                  to={`/app/dynasty/${dynastyId}/challenge/${bossPoet.id}?boss=true`}
                  data-boss
